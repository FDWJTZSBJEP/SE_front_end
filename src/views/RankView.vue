...
const getProtocolData = async () => {
  try {
    const response = await request({
      method: "GET",
      url: "https://mock.apifox.com/m1/3807087-0-default/protocol",
    });
...
const getLanguageData = async () => {
  try {
    const response = await request({
      method: "GET",
      url: "https://mock.apifox.com/m1/3807087-0-default/language",
    });
...
const getProjectData = async () => {
  try {
    const response = await request({
      method: "GET",
      url: "https://mock.apifox.com/m1/3807087-0-default/project",
    });
...
onMounted(() => {
  LanguageData.value = LanguageDataTest
...
// 渲染图表的函数
function renderDisplayChart(data: { language: string; user_num: number; rank: number }[]){
  const languages = data.map(item => item.language);
  const users = data.map(item => item.user_num);
...
.el-table .cell {
  font-size: 20px; /* 字体大小 */
  font-family: "Arial Blod", sans-serif; /* 字体样式 */
