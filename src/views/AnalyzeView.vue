<template>
  <el-tabs type="border-card">
    ...
    <el-tab-pane label="语言项目">
      <div id="bar-chart" style="width: 600px; height: 700px;"></div>
    </el-tab-pane>

    <el-tab-pane label="国家用户">
      <div>
        <h1>国家用户分析图</h1>
        <div id="pie-chart" style="width: 800%; height: 700px;"></div>
      </div>
    </el-tab-pane>
    ...
    <el-tab-pane label="语言收藏">
      <h1>语言收藏量分析图</h1>
      <div id="star-language-chart" style="width: 1000px; height: 600px;"></div>
...
const getTimeOperateCommitData = async () => {
  try {
    const response = await request({
      method: "GET",
      url: "https://mock.apifox.com/m1/3807087-0-default/TimeOperateCommit",
    });
...
const getStarLanguageData = async () => {
  try {
    const response = await request({
      method: "GET",
      url: "https://mock.apifox.com/m1/3807087-0-default/language_star",
    });
